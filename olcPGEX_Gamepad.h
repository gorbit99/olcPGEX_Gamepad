...
#pragma endregion
    ...
    private:
        ...
        constexpr static const int32_t axisCodes[GP_AXIS_COUNT]{
                ABS_Y, ABS_X,  ABS_RY,    ABS_RZ,
                ABS_Z, ABS_RX, ABS_HAT0X, ABS_HAT0Y};
#endif
#ifdef __EMSCRIPTEN__
        long id;
...
void olc::GamePad::init() {
    ...
    deviceEnumThread = std::thread{[&]() {
        auto enumFunction = [](LPCDIDEVICEINSTANCEA lpddi, LPVOID pvRef) {
            ...
            auto result =
                    reinterpret_cast<std::vector<olc::GamePad *> *>(pvRef);
        ...
        while (true) {
            ...
            dev->EnumDevices(DI8DEVCLASS_GAMECTRL, enumFunction,
                             (LPVOID)&gamepads, DIEDFL_ALLDEVICES);
        }
    }};
...
inline olc::GamePad::GamePad(LPCDIDEVICEINSTANCEA lpddi) {
    ...
    auto enumDOFunction = [](LPCDIDEVICEOBJECTINSTANCEA lpddoi, LPVOID pvRef) {
        ...
        std::tie(thisDevice, axisPositions) = *reinterpret_cast<
                std::tuple<IDirectInputDevice8A *, std::vector<int> *> *>(
                pvRef);
    ...
    if (!xInput) {
        ...
        for (size_t i = 0; i < items; i++) {
            ...
            if (data[i].dwOfs >= (DWORD)FIELD_OFFSET(GamePadState, buttons[0])
                && data[i].dwOfs
                           <= (DWORD)FIELD_OFFSET(GamePadState, buttons[19])) {
            ...
            if (data[i].dwOfs >= (DWORD)FIELD_OFFSET(GamePadState, axes[0])
                && data[i].dwOfs
                           <= (DWORD)FIELD_OFFSET(GamePadState, axes[5])) {
                int id =
                        ((data[i].dwOfs - offsetof(GamePadState, axes[0])) / 4);
...
void olc::GamePad::poll() {
    ...
    while (readEvent(event)) {
        if (event.type == EV_KEY
            && ((event.code & BTN_GAMEPAD) == BTN_GAMEPAD
...
void olc::GamePad::init() {
    ...
    auto gamepadCallback = [](int eventType,
        ...
        if (eventType == EMSCRIPTEN_EVENT_GAMEPADCONNECTED) {
            if (gamepads->size() <= gamepadEvent->index) {
                gamepads->push_back(new olc::GamePad{
                        gamepadEvent->index, std::string{gamepadEvent->id}});
            } else {
    ...
    emscripten_set_gamepaddisconnected_callback(&gamepads, true,
                                                gamepadCallback);
}
#endif
